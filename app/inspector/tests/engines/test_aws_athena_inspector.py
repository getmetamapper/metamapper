...
import app.inspector.engines.aws_athena_inspector as engine
import botocore.exceptions as exceptions


class AwsAthenaInspectorTests(unittest.TestCase):
    ...
    @mock.patch.object(engine.AwsAthenaInspector, '_list_data_catalogs')
    def test_verify_connection(self, mock_list_catalogs):
    ...
    @mock.patch.object(engine.AwsAthenaInspector, '_list_data_catalogs')
    def test_verify_connection_empty(self, mock_list_catalogs):
        """It should return False when the catalog does not exist.
        """
        mock_list_catalogs.return_value = [
            {
                'CatalogName': 'aperture',
            },
            {
                'CatalogName': 'NotAcme',
            }
    ...
    @mock.patch.object(engine.AwsAthenaInspector, '_list_data_catalogs')
    def test_verify_connection_on_param_error(self, mock_list_catalogs):
        """It should return False when the catalog does not exist.
        """
        mock_list_catalogs.side_effect = exceptions.ParamValidationError(report=None)

        self.assertFalse(self.engine.verify_connection())

    @mock.patch.object(engine.AwsAthenaInspector, '_list_data_catalogs')
    def test_verify_connection_on_creds_error(self, mock_list_catalogs):
        """It should return False when the catalog does not exist.
        """
        mock_list_catalogs.side_effect = exceptions.NoCredentialsError()

        self.assertFalse(self.engine.verify_connection())

    @mock.patch.object(engine.AwsAthenaInspector, '_list_datasets')
    @mock.patch.object(engine.AwsAthenaInspector, '_list_table_metadata')
    def test_get_tables_and_views(self, mock_list_tables, mock_list_datasets):
        ...
        mock_list_tables.return_value = [
            {'Name': 'table_1', 'TableType': 'VIEW', 'Columns': [], 'PartitionKeys': []},
            {'Name': 'table_2', 'TableType': 'VIEW', 'Columns': [], 'PartitionKeys': []},
            {'Name': 'table_3', 'TableType': 'VIEW', 'Columns': [], 'PartitionKeys': []},
            {'Name': 'table_4', 'TableType': 'VIEW', 'Columns': [], 'PartitionKeys': []},
        ]
    ...
    def test_get_table(self):
        ...
        for i in range(4):
            col = {
                ...
                'Comment': 'description_%s' % str(i).zfill(3),
            }
            schema.append(col)
